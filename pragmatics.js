...
var meaning = prob(function(utterance)
{
    if(utterance == "foo"){return prob(function(w){return w})} //"foo" is true only in true worlds.
    return prob(function(w){return true}) //"bar" is always true
})

var listener = prob(function(utterance) {
    var world = flip(); //world prior
    //speaker(world, utterance)
    condition(speaker(world,utterance) == utterance)
    return world
})


var speaker = marginalizeConditioned(
                        prob(function(world) {
                          var utterance = flip() ? "foo" : "bar"
                          //litlistener(utterance, world)
                          condition(litlistener(utterance,world) == world)
                          return utterance
                          }), samplefn, samplesPerLevel)


var litlistener = marginalizeConditioned(
                            prob(function(utterance)
                              {
                              var world = flip(); //world prior
...
var comp = prob(function() {return listener("bar")})
